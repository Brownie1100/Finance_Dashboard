...
export function IncomeFormDebug({ onIncomeAdded }: IncomeFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    console.log("API URL:", `http://localhost:8282/api/income`)
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(`Failed to add income: ${response.status} - ${responseText}`)
      }
    } catch (error) {
      console.error("Error adding income:", error)
      toast({
        title: "Error",
        description: `Failed to add income: ${error instanceof Error ? error.message : "Unknown error"}`,
        variant: "destructive",
      })
    } finally {
  ...
  return (
    <div className="space-y-4">
      <div className="p-4 bg-blue-50 rounded-lg">
        ...
        <p className="text-sm text-blue-600">User ID: {userId || "Not logged in"}</p>
        <p className="text-sm text-blue-600">API URL: http://localhost:8282/api/income/{userId}</p>
      </div>
